...
def main(dbname):
    ...
    if (row[column['a_applicationname']]) == "msedge":
            ...
            if (row[column['u_type']])=="Kombinationsfeld":
                lib2_bausteine.a_click_kombinationsfeld(xaml,str(row[column['a_applicationname']]),url, str(row[column['u_name']]), str(row[column['automationid']]))
            ...
            #  gesonderte Bausteine für Klicks in Applikation
                ...
                if (row[column['u_type']]) == "Bearbeiten":
                    ...
                    if (row[column['u_eventtype']]) == "Left-Down":
                        ...
                        lib2_bausteine.a_click_right_in_application[column['a_applicationname']]),str(row[column['a_windowtitle']]),str(row[column['automationid']]))
